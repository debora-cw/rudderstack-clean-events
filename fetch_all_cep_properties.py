...
def is_cep(name: str) -> bool:
    """Verifica se o nome segue o padrão de CEP brasileiro."""
    cep_pattern = r'^\d{5}-\d{3}$'
    return bool(re.match(cep_pattern, name))

def fetch_all_properties() -> List[Dict]:
    """Busca todas as propriedades da API do RudderStack com paginação."""
    url = f"{BASE_URL}/catalog/properties"
    all_properties = []
    page = 1
    page_size = 50  # ou o máximo permitido pela API
    while True:
        ...
        try:
            ...
            if isinstance(response_data, dict) and 'data' in response_data:
                ...
                if not properties:
                    break  # Não há mais propriedades
...
def filter_cep_properties(properties: List[Dict]) -> List[Dict]:
    """Filtra apenas as propriedades que seguem o padrão de CEP."""
    cep_properties = []
...
def main():
    ...
    if not all_properties:
        ...
        return
    # Filtra apenas as propriedades que são CEPs
    ...
    # Salva as propriedades de CEP em um arquivo
